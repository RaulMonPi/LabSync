...
const WIND_MIN_INTERVAL_MS = 5000;  // ms
const WIND_MAX_INTERVAL_MS = 9000;  // ms
...
function updateGame() {
  ...
  if (!windInProgress && game.time.now >= windNextAt) {
    let dir = (game.rnd.integerInRange(0, 1) === 0) ? 'left' : 'right';
    triggerWind(dir);
  }
